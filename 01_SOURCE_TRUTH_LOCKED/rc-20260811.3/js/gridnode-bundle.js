...
function setScannerMode(mode, button) {
  ...
  if (asset) {
    const frontAsset = asset.dataset.front || '/assets/scanner-mannequin-front.webp';
    const backAsset = asset.dataset.back || '/assets/scanner-mannequin-rear.webp';
    asset.dataset.front = frontAsset;
    asset.dataset.back = backAsset;
    asset.src = moduleState.scannerMode === 'upper' ? backAsset : frontAsset;
    asset.alt = moduleState.scannerMode === 'upper'
      ? tx('shots.scannerAltRear', 'GRID//NODE rear upper-arm diagnostic scan')
      : tx('shots.scannerAltFront', 'GRID//NODE front diagnostic scan');
  }
...
function selectScannerLocation(label) {
  ...
  // If the SHOT sheet is open, keep location in sync without discarding draft.
...
function registerServiceWorker() {
  ...
  navigator.serviceWorker
    .register('/sw.js?v=20260811.3', { updateViaCache: 'none' })
