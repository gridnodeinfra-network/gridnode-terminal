...
function setScannerMode(mode, button) {
  ...
  if (asset) {
    const coreAsset = asset.dataset.core || '/assets/scanner-core.svg';
    const lowerAsset = asset.dataset.lower || '/assets/scanner-lower.svg';
    const upperAsset = asset.dataset.upper || '/assets/scanner-upper.svg';
    asset.dataset.core = coreAsset;
    asset.dataset.lower = lowerAsset;
    asset.dataset.upper = upperAsset;
    // purpose-built views — never the legacy baked-zone body
    const src = moduleState.scannerMode === 'upper' ? upperAsset
      : moduleState.scannerMode === 'lower' ? lowerAsset
      : coreAsset;
    asset.src = src;
...
function registerServiceWorker() {
  ...
  navigator.serviceWorker
    .register('/sw.js?v=20260811.5', { updateViaCache: 'none' })
