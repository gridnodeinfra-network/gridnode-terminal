...
/* === v0.15.15 SCANNER UX POLISH: audio + haptic helpers === */
let _gnAudioCtx = null;
function gnAudio(type) {
  try {
    if (!_gnAudioCtx) {
...
function scannerSkinTone() {
  try {
    const v = localStorage.getItem('gn_scanner_skin_tone');
...
function setScannerSkinTone(tone, button) {
  if (tone !== 'light' && tone !== 'medium' && tone !== 'deep') tone = 'light';
  try { gnAudio('tick'); } catch (e) {}
  try { localStorage.setItem('gn_scanner_skin_tone', tone); } catch (e) {}
  qa('.scanner-skin-btn').forEach(item => {
    const active = item === button || item.dataset.skin === tone;
    item.classList.toggle('active', active);
    item.setAttribute('aria-pressed', active ? 'true' : 'false');
  });
  // Re-render the body asset to use the new skin tone
  if (typeof setScannerMode === 'function') {
    setScannerMode(moduleState.scannerMode, null);
  } else if (typeof renderScanner === 'function') {
    renderScanner();
  }
}

function scannerIntensityForMode(mode) {
  try {
    const modeZones = (typeof ZONES !== 'undefined' && ZONES[mode]) || [];
    if (!modeZones.length) return 'light';
    if (moduleState.selectedLocation && modeZones.indexOf(moduleState.selectedLocation) !== -1) return 'medium';
    const recent = sortedShots().slice(-6).reverse().map(item => item.site).filter(Boolean);
    const uniqRecent = Array.from(new Set(recent));
    const lastSite = uniqRecent[0] || '';
    if (lastSite && modeZones.indexOf(lastSite) !== -1) return 'deep';
    if (uniqRecent.some(site => modeZones.indexOf(site) !== -1)) return 'medium';
  } catch (e) {}
  return 'light';
}
...
function setScannerMode(mode, button) {
  ...
  if (stage) {
    stage.classList.remove('mode-core', 'mode-lower', 'mode-upper');
    stage.classList.add(`mode-${moduleState.scannerMode}`);
    stage.dataset.view = moduleState.scannerMode;
    stage.classList.remove('gn-scan-channel');
    // force reflow for channel transition
    void stage.offsetWidth;
    stage.classList.add('gn-scan-channel');
    clearTimeout(stage._channelTimer);
    stage._channelTimer = setTimeout(() => stage.classList.remove('gn-scan-channel'), 320);
  }
  const asset = $('scannerBodyAsset');
  if (asset) {
    // 9-state asset map: 3 body parts (core/legs/arms) x 3 intensities (light/medium/deep)
    const folder = moduleState.scannerMode === 'upper' ? 'arms'
                 : moduleState.scannerMode === 'lower' ? 'legs'
                 : 'core';
    const skinTone = scannerSkinTone();
    asset.src = `/assets/scanner/${folder}/${folder}-${skinTone}.webp`;
    asset.alt = moduleState.scannerMode === 'upper'
      ? tx('shots.scannerAltRear', 'GRID//NODE rear upper-arm diagnostic scan')
      : moduleState.scannerMode === 'lower'
        ? tx('shots.scannerAltLower', 'GRID//NODE front thigh diagnostic scan')
        : tx('shots.scannerAltFront', 'GRID//NODE front abdomen diagnostic scan');
  }
  ...
  if (helper) {
    const help = {
      core: tx('shots.coreHint', 'CORE focuses the abdomen. Tap a quadrant to stage your next SHOT location.'),
      lower: tx('shots.lowerHint', 'LOWER focuses the front thighs. Tap an upper or lower thigh zone.'),
      upper: tx('shots.upperHint', 'UPPER shows the back of the upper arms. Tap a triceps zone above the elbow.')
    };
...
function selectScannerLocation(label) {
  ...
  qa('.gn-zone-path').forEach(path => {
    const on = path.getAttribute('data-site') === label;
    path.classList.toggle('selected', on);
    path.classList.toggle('selected-active', on);
    if (on) {
      path.classList.remove('zone-confirming');
      void path.getBBox?.();
      path.classList.add('zone-confirming');
      setTimeout(() => path.classList.remove('zone-confirming'), 560);
    }
  });
  const activeZone = document.querySelector(`.zone-overlay[data-site="${CSS.escape(label)}"]`);
  if (activeZone) {
    activeZone.classList.add('zone-confirming');
    setTimeout(() => activeZone.classList.remove('zone-confirming'), 460);
  }
  if (navigator.vibrate) navigator.vibrate([4, 10, 4]);  /* v0.15.15: toned down to premium subtle */
...
function renderScanner() {
  try {
    if (!localStorage.getItem('gn_scanner_hint_shown')) {
      setTimeout(() => {
        ...
        const zone = document.querySelector('.gn-stable-zone-btn') || document.querySelector('.zone-overlay[data-mode="' + moduleState.scannerMode + '"]');
  ...
  qa('.zone-overlay').forEach(button => {
    ...
    button.classList.toggle('selected', site === moduleState.selectedLocation);
    button.classList.toggle('selected-active', site === moduleState.selectedLocation && visible);
    button.classList.toggle('last', Boolean(lastSite && site === lastSite && site !== moduleState.selectedLocation));
    button.classList.toggle('recent', Boolean(site && uniqRecent.includes(site) && site !== lastSite && site !== moduleState.selectedLocation));
    button.setAttribute('aria-pressed', site === moduleState.selectedLocation ? 'true' : 'false');
  });
  qa('.gn-zone-path').forEach(path => {
    const site = path.getAttribute('data-site');
    const mode = path.getAttribute('data-mode');
    const visible = mode === moduleState.scannerMode;
    ...
    path.classList.toggle('selected-active', isSel && visible);
    ...
    // dim unselected siblings of the same mode once a selection exists
    path.classList.toggle('is-dim', Boolean(visible && moduleState.selectedLocation && !isSel));
  });
  // Keep body-part image in sync with current state (selected -> medium, last -> deep, recent -> medium, none -> light)
  const _asset = $('scannerBodyAsset');
  if (_asset) {
    const _folder = moduleState.scannerMode === 'upper' ? 'arms'
                  : moduleState.scannerMode === 'lower' ? 'legs'
                  : 'core';
    const _skinTone = scannerSkinTone();
    const _desired = `/assets/scanner/${_folder}/${_folder}-${_skinTone}.webp`;
    if (_asset.getAttribute('src') !== _desired) _asset.src = _desired;
  }
}
...
function initModules() {
  document.addEventListener('click', event => {
    ...
    if (zone) { if (navigator.vibrate) navigator.vibrate(15); gnAudio('tap'); selectScannerLocation(zone.dataset.stableZone); }
    const overlay = event.target.closest('.zone-overlay');
    if (overlay?.dataset.site) { if (navigator.vibrate) navigator.vibrate(15); gnAudio('tap'); selectScannerLocation(overlay.dataset.site); }
    ...
    if (event.target.closest('[role="switch"], [role="checkbox"], .switch, .toggle, [data-toggle], [data-switch]')) { gnAudio('tick'); }
