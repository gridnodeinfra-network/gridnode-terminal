...
function setScannerSkinTone(tone, button) {
  ...
  try { gnAudio('tick'); localStorage.setItem('gn_scanner_skin_tone', tone); } catch (e) {}
...
export function setScannerMode(mode, button) {
  ...
  if (helper) {
    const help = {
      ...
      lower: tx('shots.lowerHint', 'LOWER focuses the thighs. Tap an upper or lower thigh zone.'),
      upper: tx('shots.upperHint', 'UPPER shows the back of the upper arms. Tap the matching rear-arm zone.')
    };
...
export function selectScannerLocation(label) {
  ...
  if ($('logOv')?.classList.contains('active')) setText('modalSelectedLocation', zoneLabel(label));
}
