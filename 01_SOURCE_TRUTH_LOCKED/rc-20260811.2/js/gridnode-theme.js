(() => {
  ...
  function makeLangControl(context) {
    ...
    // R2.2 rev (2026-08-11): crisp vector 電 (electricity) — resolution-
    // independent stroke path, not font text. EN=cyan, ES=Mars Red.
    const active = currentLang();
    const kanjiPath = '<svg class="gn-lang-kanji" viewBox="0 0 24 24" width="20" height="20" fill="none" aria-hidden="true"><path d="M4.5 5.2h15M7.2 5.2v2.4c0 1.1.5 2.1 1.4 2.8l2.4 1.7-2.4 1.7c-.9.7-1.4 1.7-1.4 2.8v2.6M16.8 5.2v2.4c0 1.1-.5 2.1-1.4 2.8l-2.4 1.7 2.4 1.7c.9.7 1.4 1.7 1.4 2.8v2.6M9.2 18.8h5.6M12 12.1v6.7" stroke="currentColor" stroke-width="1.85" stroke-linecap="round" stroke-linejoin="round"/></svg>';
    wrap.innerHTML =
      '<button type="button" class="gn-lang-globe" data-lang-choice="' + (active === 'es' ? 'en' : 'es') + '" aria-label="' + (active === 'es' ? 'English' : 'Español') + '" title="' + (active === 'es' ? 'Switch to English' : 'Cambiar a Español') + '" aria-pressed="false">' + kanjiPath + '</button>';
