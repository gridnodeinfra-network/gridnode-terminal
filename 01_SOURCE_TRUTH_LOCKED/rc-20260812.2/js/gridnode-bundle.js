...
function setScannerMode(mode, button) {
  ...
  if (asset) {
    ...
    const folder = moduleState.scannerMode === 'upper' ? 'arms'
                 ...
                 : 'core';
    const intensity = scannerIntensityForMode(moduleState.scannerMode);
    asset.src = `/assets/scanner/${folder}/${folder}-${intensity}.png`;
...
function renderScanner() {
  ...
  if (_asset) {
    const _folder = moduleState.scannerMode === 'upper' ? 'arms'
                  : moduleState.scannerMode === 'lower' ? 'legs'
                  : 'core';
    const _intensity = scannerIntensityForMode(moduleState.scannerMode);
    const _desired = `/assets/scanner/${_folder}/${_folder}-${_intensity}.png`;
...
function registerServiceWorker() {
  ...
  navigator.serviceWorker
    .register('/sw.js?v=20260812.2', { updateViaCache: 'none' })
