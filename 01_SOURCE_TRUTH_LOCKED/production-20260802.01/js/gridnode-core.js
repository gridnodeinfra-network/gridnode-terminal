...
const MEDICATION_ALIASES = Object.freeze({
  zepbound_tirzepatide: 'zepbound_tirzepatide', zepbound: 'zepbound_tirzepatide', 'zepbound (tirzepatide)': 'zepbound_tirzepatide',
  mounjaro_tirzepatide: 'mounjaro_tirzepatide', mounjaro: 'mounjaro_tirzepatide', 'mounjaro (tirzepatide)': 'mounjaro_tirzepatide',
  tirzepatide_compound: 'tirzepatide_compound', tirzepatide: 'tirzepatide_compound', 'tirzepatide compound': 'tirzepatide_compound', 'tirzepatide (compound)': 'tirzepatide_compound',
...
export function normalizeMedicationId(value) {
  return MEDICATION_ALIASES[String(value || '').trim().toLowerCase()] || '';
}

export function normalizeSideEffectId(value) {
  const raw = String(value || '').trim();
  return ['nausea', 'fatigue', 'headache', 'diarrhea', 'constipation', 'vomiting', 'insomnia', 'bloating', 'reflux', 'dizziness'].includes(raw.toLowerCase()) ? raw.toLowerCase() : raw;
}
...
export function formatDate(value, options = { month: 'short', day: 'numeric', year: 'numeric' }) {
  ...
  const locale = typeof document !== 'undefined' && document.documentElement?.lang?.startsWith('es') ? 'es-419' : 'en-US';
...
export function todayISO(date = new Date()) { return localISODate(date); }
