...
(function () {
  ...
  function layerIsOpen(layer) {
    ...
    const activeClass = layer.classList.contains('active') || layer.classList.contains('gn-onb-overlay');
    if (!activeClass) return false;
    const style = getComputedStyle(layer);
    return style.display !== 'none' && style.visibility !== 'hidden';
  }

  function dismissLayer(layer, { preserveShotDraft = false } = {}) {
    if (preserveShotDraft && layer?.id === 'logOv') {
      captureShotDraft();
    ...
    const dismiss = layer?.querySelector(OVERLAY_CLOSE_SELECTOR);
    if (dismiss) { dismiss.click(); return; }
    layer?.classList.remove('active');
    if (layer && getComputedStyle(layer).display !== 'none') layer.style.display = 'none';
    layer?.setAttribute('aria-hidden', 'true');
  }

  function pushLayerHistory(layer, id) {
    const push = () => {
      if (layerIsOpen(layer) && history.state?.gnLayer !== id) history.pushState({ ...(history.state || {}), gnLayer: id }, '');
    };
    if (handlingPop) window.setTimeout(push, 0);
    else push();
  }
  ...
  function wireNavigationStack() {
    ...
    window.addEventListener('popstate', event => {
      ...
      const openOverlay = Array.from(document.querySelectorAll(OVERLAY_SELECTOR)).filter(layerIsOpen).at(-1);
      if (openOverlay) {
        ...
        dismissLayer(openOverlay, { preserveShotDraft: openOverlay.id === 'logOv' });
  ...
  function wireLayerHistory() {
    ...
    const bind = layer => {
      ...
      if (active) {
        layer.dataset.gnHistoryOpen = 'true';
        pushLayerHistory(layer, layer.id || 'layer');
      }
    };
    document.querySelectorAll(OVERLAY_SELECTOR).forEach(bind);
    const reconcileRemovedLayer = layer => {
      if (!layer?.dataset?.gnHistoryOpen || handlingPop || history.state?.gnLayer !== (layer.id || 'layer')) return;
      consumeInternalHistoryPop = true;
      history.back();
    };
    new MutationObserver(records => records.forEach(record => {
      ...
      record.removedNodes.forEach(node => {
        if (!(node instanceof Element)) return;
        if (node.matches?.(OVERLAY_SELECTOR)) reconcileRemovedLayer(node);
        node.querySelectorAll?.(OVERLAY_SELECTOR).forEach(reconcileRemovedLayer);
      });
    })).observe(document.body, { childList: true, subtree: true });
  }
  ...
  function wireOverlayFocus() {
    const selector = OVERLAY_SELECTOR + ', .gn-lab-tool-overlay';
    const focusableSelector = 'button,[href],input,select,textarea,[tabindex]:not([tabindex="-1"])';
    const dialogSurface = overlay => overlay.querySelector(':scope > [role="dialog"]') || overlay;
    const visibleFocusables = overlay => Array.from(overlay.querySelectorAll(focusableSelector)).filter(element => {
      if (element.disabled || element.hidden || element.closest('[hidden],.gn-hidden,[inert]')) return false;
      const style = getComputedStyle(element);
      return style.visibility !== 'hidden' && style.display !== 'none' && element.getClientRects().length > 0;
    });
    const ensureDialogSemantics = overlay => {
      const surface = dialogSurface(overlay);
      if (surface !== overlay) {
      ...
      if (surface.getAttribute('role') === 'dialog') surface.setAttribute('aria-modal', 'true');
      if (surface.hasAttribute('aria-label') || surface.hasAttribute('aria-labelledby')) return;
      const title = surface.querySelector('.modal-title,.gn-import-title,.archive-confirm-title,.future-confirm-title,.gn-whatsnew-title,.gn-peptide-overlay-title,h1,h2,h3');
      if (title) {
        if (!title.id) title.id = (overlay.id || 'gnDialog') + 'Title';
        surface.setAttribute('aria-labelledby', title.id);
      } else surface.setAttribute('aria-label', copy('GRID//NODE dialog', 'Diálogo GRID//NODE'));
    };
    const syncFocus = overlay => {
      const active = layerIsOpen(overlay);
      if (active && !overlay.dataset.gnFocusOpen) {
        overlay.dataset.gnFocusOpen = 'true';
        const current = document.activeElement;
        overlay._gnReturnFocus = current && current !== document.body && !overlay.contains(current) ? current : null;
        window.requestAnimationFrame(() => {
          if (!layerIsOpen(overlay) || overlay.contains(document.activeElement)) return;
          const target = visibleFocusables(overlay)[0];
      ...
      } else if (!active && overlay.dataset.gnFocusOpen) {
        delete overlay.dataset.gnFocusOpen;
        const target = overlay._gnReturnFocus;
        overlay._gnReturnFocus = null;
        window.requestAnimationFrame(() => target?.isConnected && target.focus?.());
      }
    };
    const focusObserver = new MutationObserver(records => records.forEach(record => syncFocus(record.target)));
    const bind = overlay => {
      if (overlay.dataset.gnNativeBound) return;
      overlay.dataset.gnNativeBound = 'true';
      ensureDialogSemantics(overlay);
      focusObserver.observe(overlay, { attributes: true, attributeFilter: ['class', 'style', 'aria-hidden'] });
      overlay.addEventListener('keydown', event => {
        if (event.key === 'Escape') {
          ...
          dismissLayer(overlay);
          return;
        }
        if (event.key !== 'Tab' || !layerIsOpen(overlay)) return;
