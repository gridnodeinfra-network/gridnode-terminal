...
/* GRID//NODE pk-curve edge function.
 * Server-side home of the Peptide Evidence Engine: the curated, primary-
 * source-derived PK/PD model table plus the protocol-curve math. The
 * browser never sees the model parameters; it receives computed points.
 * verify_jwt = false — pure computation over caller-supplied shot records,
 * no user data touched. CORS-locked to GRID//NODE origins.
 *
 * POST /functions/v1/pk-curve
 *   { medId, shots: [{date, dose, med, archived}], rangeDays, pointCount, now }
 * -> { points, nowIndex, markers, start, end, now, count, medId,
 *      evidence: {state}, modelKind, tier, horizonDays, curvePhase, dossier }
 */
...
/* Curve-phase label for the most recent shot (presentation aid; the client
 * localizes the label itself). */
function curvePhaseFor(model, elapsedH) {
...
function dossierOf(model) {
  const d = {
    notes: Array.isArray(model.notes) ? model.notes : [],
    sources: Array.isArray(model.sources) ? model.sources : [],
    animal: model.animal || null,
  };
  if (model.kind === 'curve') {
    d.riseToPeakH = model.riseToPeakH;
    d.peakWindowH = model.peakWindowH;
    d.tHalfH = model.tHalfH;
  }
  if (model.kind === 'window' && Array.isArray(model.windows)) {
    d.windows = model.windows;
  }
  return d;
}
...
serve(async (req) => {
  ...
  if (!medId) return json(req, { error: "medId required" }, 400);
  const rawShots = Array.isArray(body.shots) ? body.shots : [];
  const shots = rawShots.slice(0, 2000).map((s) => ({
    date: String(s.date || ""),
    dose: Number(s.dose) || 0,
    med: String(s.med || ""),
    archived: !!s.archived,
  }));
  const pointCount = Math.min(400, Math.max(16, Number(body.pointCount) || 96));
  const now = Number(body.now) || Date.now();

  const build = buildProtocolCurve(shots, medId, body.rangeDays ?? 30, now, pointCount);
  const model = build.model;
  const last = build.markers.filter((m) => !m.planned).at(-1);
  const elapsedH = last ? (build.now - last.t) / 3600000 : 0;

  return json(req, {
    points: build.points,
    nowIndex: build.nowIndex,
    ...
    count: build.count,
    medId: build.medId,
    evidence: { state: evidenceOf(model).state },
    modelKind: model.kind,
    tier: model.tier,
    horizonDays: model.horizonDays || 7,
    curvePhase: curvePhaseFor(model, elapsedH),
    dossier: dossierOf(model),
  });
});
